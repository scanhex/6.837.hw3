...
struct ParticleState {
  ...
  ParticleState(){}
  explicit ParticleState(const std::vector<glm::vec3>& positions) : positions(positions), velocities(positions.size()) {}
  explicit ParticleState(std::vector<glm::vec3> positions, std::vector<glm::vec3> velocities) : positions(std::move(positions)), velocities(std::move(velocities)) {}
